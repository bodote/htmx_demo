...
def test_init():
    contact_instance: Contact = Contact(id_=1, first="Bodo", last=None, phone=None, email=None)
...
def test_search():
    Contact.load_db()
    found_list: List[Contact] = Contact.search("Joe")
...
def test_find_one():
    Contact.load_db()
    contact:Optional[Contact]  = Contact.find(5)
...
def test_delete_one():
    ...
    contact_deleted:Optional[Contact]  = Contact.find(5)
